...
if [[ "$INSTALL_NFS_CLIENT" == "y" || "$INSTALL_NFS_CLIENT" == "yes" ]]; then
    ...
    case "$DISTRO" in
        ...
        centos|rhel|fedora)
            PKG="nfs-utils"
            CMD="dnf install -y $PKG || yum install -y $PKG"
            ;;
        arch)
            PKG="nfs-utils"
...
# Check write access and create directories
for DIR in "$MEDIA_ROOT" "$CONFIG_ROOT"; do
    mkdir -p "$DIR"
    if ! [ -w "$DIR" ]; then
...
echo "  chown -R $USER:$USER $MEDIA_ROOT $CONFIG_ROOT"
